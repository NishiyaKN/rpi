...
def scrape_product_price(scraper, url):
    """Simplified scraping function optimized for terabyte"""
    try:
        # Realistic headers
        headers = {
        ...
        if response.status_code == 200:
            ...
            # terabyte price element
            price_element = soup.find('p', {'id': 'valVista', 'class': 'val-prod valVista'}) or \
               soup.select_one('.val-prod') or \
               soup.select_one('.precoPor')
...
def update_price_history():
    products = load_json_file('terabyte-products.json')
    if not products:
        print("❌ No products found in terabyte-products.json")
        return
        
    price_history = load_json_file('terabyte-prices.json')
    ...
    if updated:
        with open('terabyte-prices.json', 'w', encoding='utf-8') as f:
